...
if (($argv[1] ?? '') === '--worker') {
    ...
    $result=register_student_attendance($argv[2],[
        'student_code'=>'9999'.str_pad($argv[3],4,'0',STR_PAD_LEFT),
        'student_name'=>'นิสิตสมมติพร้อมกัน',
        'client_request_id'=>'parallel-request-'.$argv[3],
    ]);
...
// Keep a tiny legacy capacity value to prove that it is no longer an admission limit.
db()->exec('UPDATE rooms SET capacity=2 WHERE id='.$room);

$created=create_class_session([
    'room_id'=>$room,
    'lecturer_user_id'=>$admin,
    'course_code'=>'RACE101',
    'course_name'=>'ทดสอบพร้อมกัน',
    'starts_at'=>'2035-05-06T09:00',
    'ends_at'=>'2035-05-06T13:00',
    'checkin_mode'=>'manual',
]);
if (!$created['ok']) throw new RuntimeException('Cannot create isolated fixture');

$class=get_class_session($created['id']);
session_write_close();
...
if ($accepted!==8 || $failed!==0 || $count!==8) {
    throw new RuntimeException("Unlimited attendance race failed: accepted=$accepted failed=$failed saved=$count");
}

echo "PASS: 8 simultaneous students accepted even though legacy room capacity is 2; no artificial capacity limit\n";
